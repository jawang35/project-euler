...
answer =
    ...
    where allConsecutiveQuadraticPrimes = [ (a, b, consecutiveQuadraticPrimes a b)
          ...
          maximumConsecutivePrimes      = maximum $ map (\(_, _, x) -> x) allConsecutiveQuadraticPrimes
          (a, b, _)                     = fromJust $ find (\(_, _, x) -> x == maximumConsecutivePrimes) allConsecutiveQuadraticPrimes
