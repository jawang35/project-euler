...
isLychrel 0 _               = True
isLychrel iterations number =
    not (isPalindrome $ digits 10 nextNumber) && isLychrel (iterations - 1) nextNumber
    where nextNumber = number + (unDigits 10 . reverse $ digits 10 number)

answer :: Int
answer = length $ filter (isLychrel 50) [1..9999]
