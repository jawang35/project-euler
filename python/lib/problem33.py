...
from lib.helpers.numbers import prime_divisors, product
...
def cancel_digits(numerator, denominator):
    ...
    for i, digit in enumerate(numerator_string):
        ...
        if j > -1:
            simplified_numerator = int(numerator_string[:i] +
                                       numerator_string[i + 1:])
            simplified_denominator = int(denominator_string[:j] +
                                         denominator_string[j +
                                         1:])
...
def digit_cancelling_fractions(digits):
    ...
    for denominator in range(minimum, maximum):
        for numerator in range(minimum, denominator):
            ...
            for simplified_numerator, simplified_denominator \
                    in simplified_fractions:
                divisor = numerator / simplified_numerator
                if (denominator / simplified_denominator == divisor and
                        divisor % minimum > 0):
