...
def pandigital_products(max_digits):
    ...
    for multiplication_index in range(1, int(equal_index / 2) + 1):
        for multiplicand_digits in permutations(digits, multiplication_index):
            possible_multiplier_digits = [digit for digit in digits if digit not in multiplicand_digits]
            for multiplier_digits in permutations(possible_multiplier_digits, equal_index - multiplication_index):
                multiplicand = int(''.join([str(digit) for digit in multiplicand_digits]))
                multiplier = int(''.join([str(digit) for digit in multiplier_digits]))
                product = multiplicand * multiplier
                if sorted(str(multiplicand) + str(multiplier) + str(product)) == sorted_digits:
